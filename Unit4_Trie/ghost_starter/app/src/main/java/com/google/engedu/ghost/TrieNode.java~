...
public class TrieNode {
    ...
    public String getGoodWordStartingWith(String prefix) {
        ...
        /*
        // create a list for nonWord nodes and nodes that are words
        ArrayList<Map.Entry> nonWords = new ArrayList<>();
        ArrayList<Map.Entry> words = new ArrayList<>();

        // iterate through all the children of the current root
        Iterator it = currentRoot.children.entrySet().iterator();
        while (it.hasNext()) {
            Map.Entry pair = (Map.Entry) it.next();
            // if the current node is a word
            if (((TrieNode) pair.getValue()).isWord) {
                // add it to the words ArrayList
                words.add(pair);
            } else {
                // otherwise add it to the nonWords ArrayList
                nonWords.add(pair);
            }
        }

        // if the nonWords ArrayList is empty
        if (nonWords.isEmpty()) {
            // choose a random word from the words ArrayList
            int randInt = rand.nextInt(words.size());
            char newKey = (Character) words.get(randInt).getKey();
            // add the random key from words to the prefix
            prefix += newKey;
        } else {
            // choose a random word from the nonWords ArrayList
            int randInt = rand.nextInt(nonWords.size());
            char newKey = (Character) nonWords.get(randInt).getKey();
            // add the random key from nonWords to the prefix
            prefix += newKey;
        }

        // return the prefix which is the old prefix + one character
        // it does not return a completed word, but returns exactly as much characters as the
        // GhostActivity needs
        return prefix;
        */
